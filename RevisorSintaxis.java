...
public class RevisorSintaxis {
	...
	public boolean isSyntaxOk() {
		...
		while(i<operacion.length()) { 
			...
			case '(': if(i>0) {
				if(!(operacion.charAt(i-1)=='+' || operacion.charAt(i-1)=='-' || operacion.charAt(i-1)=='*'||operacion.charAt(i-1)=='/'||operacion.charAt(i-1)=='^'||operacion.charAt(i-1)=='(')) {
				resp=false;
				i=operacion.length();
				}
			}
			...
			if(x.pop()!='(') {
				...
				break;
			case'+','-','/','*','^': if(i>0 && i<operacion.length()-1) {
				if(!(((Character.isDigit(operacion.charAt(i-1)))&& (Character.isDigit(operacion.charAt(i+1)))) || ((Character.isDigit(operacion.charAt(i-1)))&& (operacion.charAt(i+1)=='(')) || ((Character.isDigit(operacion.charAt(i+1)))&& (operacion.charAt(i-1)==')')) ||((operacion.charAt(i-1)==')')&& (operacion.charAt(i+1)=='(')))) {
					resp=false;
					i=operacion.length();
				}
			}
				...
				resp=false;
				break;
			default:if(!Character.isDigit(operacion.charAt(i))&&operacion.charAt(i)!='.') {
				resp=false;
				i=operacion.length();
			}
			}
		...
		return x.isEmpty() && resp;
	}
	
	public static void main(String[] args) {
		RevisorSintaxis a= new RevisorSintaxis("2.4+3.21");
		...
		RevisorSintaxis k= new RevisorSintaxis("()()");//parentesis vacios
